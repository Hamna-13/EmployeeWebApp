...
function EmployeeLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/api/employees/employeelogin",
        { email, password }
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-900 flex justify-center overflow-hidden">
      {/* Outer card container (same as Signup/Login) */}
      <div className="max-w-screen-xl m-0 sm:m-10 bg-white shadow sm:rounded-lg flex justify-center flex-1">
        
        {/* Left form section */}
        <div className="lg:w-1/2 xl:w-5/12 p-6 sm:p-12">
          {/* Logo same style as signup/login */}
          ...
          <div className="mt-12 flex flex-col items-center">
            <h1 className="text-2xl xl:text-3xl font-extrabold">Employee Login</h1>
            ...
            <div className="w-full flex-1 mt-8">
              <form className="mx-auto max-w-xs" onSubmit={handleLogin}>
                <input
                  type="email"
                  placeholder="Email"
                  className="w-full px-8 py-4 mb-4 rounded-lg bg-gray-100 border border-gray-200 
                ...
                <button
                  type="submit"
                  className="mt-2 tracking-wide font-semibold bg-sky-600 text-white w-full py-4 
